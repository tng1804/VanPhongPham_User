...
<body>
    <div class="container">
        ...
        function callAPI_auth($method, $url, $data) // phuong thuc http,url cua api, du lieu de gui
        {
            $curl = curl_init();
            switch ($method) {
                case "POST":
                    curl_setopt($curl, CURLOPT_POST, 1); // thiet lap curl de gui 1 yeu cau post
                    if ($data) // neu co du lieu se duoc gui theo yeu cau
                ...
                default:
                    if ($data)
                        $url = sprintf("%s?%s", $url, http_build_query($data)); // gia dinh la 1 yeu cau get,neu co du lieu se duoc them vao url
            }
            // OPTIONS:
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $ispTxnRequest = array( 
                "vnp_RequestId" => $vnp_RequestId, //thuoc tinh cua giao dich
                "vnp_Version" => "2.1.0",
...
<style>
    ...
    input[value="Refund"]{
        background-color: #58257b;  border: none;  color: white;  text-align: center;  text-decoration: none;  display: inline-block;  font-size: 16px;  margin: 4px 2px;  -webkit-transition-duration: 0.4s; /* Safari */  transition-duration: 0.4s;  cursor: pointer;  border-radius: 4px;}.button1 {  background-color: white;   color: black;   border: 2px solid crimson;}.button1:hover {  background-color: crimson;  color: white;}.button8 {  background-color: midnightblue;  color: white;  border: 2px solid midnightblue;}.button8:hover {  background-color: white;  color: black;  border: 2px solid midnightblue;
    }
